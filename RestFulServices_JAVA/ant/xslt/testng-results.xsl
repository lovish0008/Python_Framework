...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="testNgXslt.systemInfo" />
    <xsl:param name="testNgXslt.reportType" />
    <xsl:param name="testNgXslt.buildNumber"/>
    <xsl:param name="testNgXslt.executedOn"/>
    <xsl:param name="testNgXslt.pass"/>
    <xsl:param name="testNgXslt.fail"/>
    <xsl:param name="testNgXslt.skip"/>
    <xsl:param name="testNgXslt.executionTime"/>
    <xsl:param name="testNgXslt.totalTests" />
    ...
    <xsl:variable name="chartWidth" select="round(435 * testng:getVariableSafe($testNgXslt.chartScaleFactor, 1))"/>
        ...
        <xsl:result-document href="{testng:absolutePath('style.css')}" format="text">
            ...
            <xsl:choose>
                <xsl:when test="testng:isFilterSelected('FAIL') = 'true'">
                    .testMethodStatusFAIL { background-color: #CB5959; }
                </xsl:when>
                <xsl:otherwise>
                    .testMethodStatusFAIL { background-color: #CB5959; display: none; }
                </xsl:otherwise>
            </xsl:choose>

            <xsl:choose>
                <xsl:when test="testng:isFilterSelected('PASS') = 'true'">
                    .testMethodStatusPASS { background-color: #70A772; }
                </xsl:when>
                <xsl:otherwise>
                    .testMethodStatusPASS { background-color: #70A772; display: none; }
                </xsl:otherwise>
            </xsl:choose>

            <xsl:choose>
                <xsl:when test="testng:isFilterSelected('SKIP') = 'true'">
                    .testMethodStatusSKIP { background-color: #F5D76C; }
                </xsl:when>
                <xsl:otherwise>
                    .testMethodStatusSKIP { background-color: #F5D76C; display: none; }
                </xsl:otherwise>
            </xsl:choose>

            <![CDATA[
           body { font-family: Arial, sans-serif; font-size: 12px; padding: 10px; margin: 0px; background:url(tiny_grid.png) repeat; }
            ...
            .suiteStatusPass { background-color: #70A772; }
            .suiteStatusFail { background-color: #CB5959; }
            ...
            .testCaseFail { background-color: #CB5959; }
            .testCasePass { background-color: #70A772; }
            .testCaseSkip { background-color: #F5D76C; }
            ...
            .testMethodsTable .testMethodStatusCONF td.firstMethodCell { border-left: 5px solid gray; }
             header {position: absolute; top:0; left:0; width:100%; border-bottom:4px solid #FB9337;}
		    #header_toolbar { background: url("topheaderbg.png") repeat scroll 0 0 #354561; border-bottom: 1px solid #303F59; height: 45px; padding-left: 10px; position: relative;}
		    .container_12 { width: auto;}
		    #header_toolbar h1 { margin: 0;}
			#header_toolbar h1 {color: #FFFFFF;font-size: 17px;padding-top: 12px;}
			...
			.footer_logo {float: right;position: relative;margin: 25px 100px 20px 0;}
			footer {background: url("topheaderbg.png") repeat scroll 0 0 #354561;border-top:2px solid #273246;height: 35px;left:0;line-height:34px;width:100%;position: absolute;bottom:0;}
			footer #button_bar {display: inline;float: right;padding-right: 2px;}
			footer #button_bar li, footer #button_bar ul {display: table-cell;padding: 3px 8px 0 0;color:#fff;}
			footer #button_bar a {display: inline-block;height: 98%;margin: 0;} 
			footer #button_bar a:active, foo0ter #button_bar a:hover {color:#fff;}
			.content {background: url("bg_mid.jpg") no-repeat scroll left bottom #FFFFFF !important;min-height: 300px;border-color: #D0D2D5 #D0D2D5 #C6C9CC;border-image: none;border-style: solid;border-width: 1px;
   				font-size: 11px;font-weight: bold;margin: 0 10px 0 0;padding: 8px 12px 6px;border-radius:5px;padding:0px 10px 10px 10px;}
			.content_rgt {-moz-border-bottom-colors: none;-moz-border-left-colors: none;-moz-border-right-colors: none;-moz-border-top-colors: none;background-color: #EFF0F0;background-image: linear-gradient(#FFFFFF, #E8EAE9);border-color: #D0D2D5 #D0D2D5 #C6C9CC;border-image: none;
   				border-style: solid;border-width: 1px;font-size: 11px;font-weight: bold;margin: 0 3px 0 0;padding: 8px 12px 6px;border-radius:5px;padding:0px 10px 10px 10px;}     
   				.right_dec{ margin:0px; padding:0px;}
				.right_dec td{ padding:12px 5px; text-align: left; border-bottom: 1px solid #CCCCCC; color:#333333; }
				.right_dec td.default{ background: #E5E9EB;text-align: left;}
				.right_dec h4{font-size:12px; font-weight:bold; margin:0px;}
   				h2 {background:#E9E9E9;border: 1px solid #D5D5D5;color: #1c3461;border-radius:2px;-webkit-border-radius:2px;-moz-border-radius:2px;padding:15px 5px;font-size: 13px;text-transform: uppercase;height: auto;}
   				.widget-box {border-bottom: 1px solid #CCCCCC;box-shadow: none;margin: 3px;padding: 0;width:45%;float:left;}
				.widget-box_full{border-bottom: 1px solid #CCCCCC;box-shadow: none;margin: 3px;padding: 0;width:100%;}
				.widget-box_rgt{border: 0px solid #CCCCCC;box-shadow: none;margin: 3px;padding: 0;width:50%;float:right;}
				.rgt_header{color:#fff;background:#2d3d5b;padding:10px;float:left;border:1px solid #2d3d5b;border-radius:2px;width:90%;}
				.rgt_header a{color:#fff;}
				.widget-header {position: relative;height: 40px;line-height: 40px;background: #2d3d5b;border-top-left-radius: 4px;border-top-right-radius: 4px;
					height: 40px;
				...
				.testCaseLink{ padding:5px 2px;border: 1px solid #dcdcdc;background:#fffef7;}
				.widget-content {background: none repeat scroll 0 0 #FFFFFF;border: 1px solid #acacac;padding: 20px 15px 15px;}
   				ul.tree{margin:0px;padding:0px;}
                li {cursor: pointer;line-height: 20px;list-style: none;}
                li ul {display: none;}
            ]]>
        </xsl:result-document>
    </xsl:template>
    
    <xsl:template name="writeJqueryJsFile">
        <xsl:result-document href="{testng:absolutePath('custom_jquery.js')}" format="text">
            <![CDATA[
    ...
    <xsl:template name="htmlHead">
        <head>
            <title>
                <xsl:value-of select="testng:getVariableSafe($testNgXslt.reportTitle, ' Test Report')"/>
            </title>
    ...
    <xsl:template match="/testng-results">
        ...
        <xsl:call-template name="writeJqueryJsFile"/>
        <html>
            <xsl:call-template name="htmlHead"/>
            <frameset rows="10%, 80%, 10%" frameborder="0">
      			<frame name="header" src="header.html"/>
        		<frameset cols="280px, 100%" frameborder="0">
        ...
        <xsl:result-document href="{testng:absolutePath('overview-chart.svg')}" format="xml">
            <svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet" height="{$chartHeight}"
                 viewBox="0 0 900 300">
                <defs>
                    <style type="text/css">
                        <![CDATA[
                ...
                <svg id="graphzone" preserveAspectRatio="xMidYMid meet" x="0" y="0">
                    ...
                    <xsl:variable name="failedCount" select="number($testNgXslt.fail)"/>
                    <xsl:variable name="passedCount" select="number($testNgXslt.pass)"/>
                    <xsl:variable name="skippedCount" select="number($testNgXslt.skip)"/>
                    <xsl:variable name="totalCount" select="number($testNgXslt.totalTests)"/>
                    ...
                    <rect style="fill:#CB5959;stroke-width:1;stroke:black;" x="10" y="86" width="20" height="20"/>
                    <text class="legendtext" x="40" y="105">Failed (<xsl:value-of select="$failedPercent"/>)
                    </text>
                    <rect style="fill:#70A772;stroke-width:1;stroke:black;" x="10" y="125" width="20" height="20"/>
                    <text class="legendtext" x="40" y="143">Passed (<xsl:value-of select="$passPercent"/>)
                    </text>
                    <rect style="fill:#F5D76C;stroke-width:1;stroke:black;" x="10" y="163" width="20" height="20"/>
		...
		<!-- Header file -->
		<xsl:result-document href="{testng:absolutePath('header.html')}" format="xhtml">
     			 ...
     			 <header>
	  				<div id="header_toolbar">
	    				<div class="container_12">
							<h1 class="grid_8"><span style="font-variant:small-caps; font-size:14px; padding:2px 14px; float:left;">QASOURCE  Application</span></h1> 
			 			</div>
					</div>
	  			</header>
   			</body>
		...
		<!--  Footer file -->
		<xsl:result-document href="{testng:absolutePath('footer.html')}" format="xhtml">
			<html xmlns="http://www.w3.org/1999/xhtml">
			<xsl:call-template name="htmlHead"/>
			<body>
  			<footer>
        		<div class="container_12">
         			<div id="button_bar">
          				<ul>
             				<li> © Copyright QASource. All Rights Reserved. www.qasource.com </li>
          				</ul>
         			 </div>
        		</div>
    		</footer>
        ...
        <xsl:result-document href="{testng:absolutePath('overview.html')}" format="xhtml">
            <html xmlns="http://www.w3.org/1999/xhtml">
                ...
                <body>
                    ...
                    <div class="widget-box_rgt">
       				  <div class="widget-header">EXECUTION SUMMARY</div>
         			  <div class="widget-content" style="padding:0px;">
                    <div class="right_dec">                    
                    		 <table width="100%" cellpadding="0" cellspacing="0"> 
                     		 <tr><td class="default"><h4>Total Test Script executed:</h4></td><td class="default"><h4><xsl:value-of select="$testNgXslt.totalTests"/></h4></td></tr>
                    	     <tr><td><h4>Total Test Script Passed:</h4></td><td><h4><xsl:value-of select="$testNgXslt.pass"/></h4></td></tr>
                         	 <tr><td class="default"><h4>Total Test Script Failed:</h4></td><td class="default"><h4><xsl:value-of select="$testNgXslt.fail"/></h4></td></tr>
                    	     <tr><td><h4>Total Test Script Skipped:</h4></td><td><h4><xsl:value-of select="$testNgXslt.skip"/></h4></td></tr>
                    	     <tr><td class="default"><h4>Executed On:</h4></td><td class="default"><h4><xsl:value-of select="$testNgXslt.executedOn"/></h4></td></tr>
                    	     <tr><td><h4>Duration:</h4></td><td><h4><xsl:value-of select="$testNgXslt.executionTime"/></h4></td></tr>
                     	     <tr><td class="default"><h4>Build Number:</h4></td><td class="default"><h4><xsl:value-of select="$testNgXslt.buildNumber"/></h4></td></tr>
                    	     <tr><td><h4>Test Environment Information:</h4></td><td><h4><xsl:value-of select="$testNgXslt.systemInfo"/></h4></td></tr>
                             </table>                     	
                    </div>
                    </div></div><br /> 
                    <div style="clear:both;"></div>  
                    <div style="clear:both;"></div><br /> 
                    <div class="widget-box_full"> 
         			<div class="widget-header">DETAILED REPORT</div> 
          			<div class="widget-content" style="padding:0px;">                  
                        ...
                        <table width="100%" cellpadding="5" cellspacing="1">
							...
							<xsl:for-each select="$testCaseElements">                                   
                                   <tr style="background-color: #dddddd; font-size: 12px;">
                                    ...
                                    <td align="center" style="font-weight: bold;" width="50px">
                                        <xsl:value-of
                ...
                <script type="text/javascript" src="custom_jquery.js"/>
            ...
            <html xmlns="http://www.w3.org/1999/xhtml">
                ...
                <div class="content_rgt">               
                    <h2 style="margin-bottom: 5px;">
                        <xsl:value-of
                        	select="testng:getVariableSafe($testNgXslt.reportTitle, ' Test Report')" />
                    </h2><div class="rgt_header">
    ...
    <xsl:template name="percentageOverview">
        ...
        <td style="background-color: #CB5959; padding: 3px 3px 3px 0;" align="center">
            <div style="width: 50px;">
                <xsl:value-of select="$failedCount"/>
            </div>
        </td>
        <td style="background-color: #70A772; padding: 3px 3px 3px 0;" align="center">
            <div style="width: 50px;">
                <xsl:value-of select="$passedCount"/>
            </div>
        </td>
        <td style="background-color: #F5D76C; padding: 3px 3px 3px 0;" align="center">
            <div style="width: 50px;">
